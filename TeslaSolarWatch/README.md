...
## STOP: In June 2026 Tesla changed their "Owners API" (used by this project) to require TLS v1.3 for the secure communication. It seems that the Google Apps Script backend uses a different version, and therefore the "access token" being generated by this code is "wrong": it results in an HTTP 403" (access forbidden) error later, when trying to collect the solar stats. See a discussion [here](https://github.com/teslamate-org/teslamate/issues/5384). There is no way for us to force the Google backends to use TLS v1.3, so there is no available fix at this time; we hope that at some point Google will add such support, and things will start working again.
